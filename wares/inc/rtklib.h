...
#define malloc(a)      rtklib_malloc((a))
#define calloc(n,s)    rtklib_calloc((n),(s))
#define realloc(p,s)   rtklib_realloc((p),(s))
#define free(p)        rtklib_free((p))
#ifdef __cplusplus
extern "C" {
#endif
...
#if defined(RTKLIB_EMBEDDED)
#define MAXSAT      (NSATGPS+NSATGLO+NSATGAL+NSATQZS+NSATCMP+NSATIRN+NSATLEO)
#else
...
#if defined(RTKLIB_EMBEDDED)
/* pcv/dgps/SBAS-corr arrays scaled with MAXSAT cost ~14 KiB each nav_t; unused on MSM+BRDC MCU */
#define NAV_LITE_AUX  1
/* UBX GPS subframe: words stored at (id-1)*30, id<=5 → ≥150 B; leave margin for decode_frame */
#ifndef RAW_SUBFRM_BYTES
#define RAW_SUBFRM_BYTES 220
#endif
#define RAW_NAV_EPH_SLOTS (MAXSAT + 12) /* broadcast ephem ring; < desktop MAXSAT*2 */
#else
#define NAV_LITE_AUX  MAXSAT
#define RAW_SUBFRM_BYTES 380
#define RAW_NAV_EPH_SLOTS (MAXSAT * 2)
#endif
...
#endif
#if defined(RTKLIB_EMBEDDED)
#define MAXRCV      8                   /* max receiver number (1 to MAXRCV) */
#else
...
#if defined(RTKLIB_EMBEDDED)
#define MAXBAND     0                   /* disable SBAS IGP table on MCU */
#define MAXNIGP     1                   /* keep minimal non-zero shape */
#else
...
#endif
#if defined(RTKLIB_EMBEDDED)
#define MAXERRMSG   512                 /* MCU: smaller rtk_t.errbuf */
#else
...
#if defined(RTKLIB_EMBEDDED)
#define MAXCODE     8                   /* trimmed for GPS-only MCU build */
#else
...
typedef struct {        /* SBAS satellite corrections type */
    ...
    sbssatp_t sat[NAV_LITE_AUX]; /* satellite correction */
} sbssat_t;
...
typedef struct {        /* navigation data type */
    ...
    pcv_t pcvs[NAV_LITE_AUX]; /* satellite antenna pcv */
    sbssat_t sbssat;    /* SBAS satellite corrections */
    sbsion_t sbsion[MAXBAND+1]; /* SBAS ionosphere corrections */
    dgps_t dgps[NAV_LITE_AUX]; /* DGPS corrections */
...
typedef struct {        /* RTCM control struct type */
    ...
    dgps_t *dgps;       /* output of dgps corrections */
                        /* SSR: use nav.ssr[] (same layout; avoids duplicating ~MAXSAT ssr_t) */
...
typedef struct {        /* receiver raw data control type */
    ...
    char msgtype[256];  /* last message type */
    uint8_t subfrm[MAXSAT][RAW_SUBFRM_BYTES]; /* subframe buffer */
