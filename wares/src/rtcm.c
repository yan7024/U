...
extern int init_rtcm(rtcm_t *rtcm)
{
...
#else
        int ne = MAXSAT * 2;
        int ng = MAXPRNGLO > 0 ? MAXPRNGLO : 1;
#endif
        rtcm->obs.data=(obsd_t *)malloc(sizeof(obsd_t)*MAXOBS);
        rtcm->nav.eph =(eph_t  *)malloc(sizeof(eph_t )*ne);
        rtcm->nav.geph=(geph_t *)malloc(sizeof(geph_t)*ng);
    }
    if (!rtcm->obs.data || !rtcm->nav.eph || !rtcm->nav.geph) {
        free_rtcm(rtcm);
        return 0;
    }
    rtcm->obs.n=0;
    ...
    rtcm->nav.n=MAXSAT*2;
#endif
    rtcm->nav.ng=MAXPRNGLO > 0 ? MAXPRNGLO : 1;
    for (i=0;i<MAXOBS   ;i++) rtcm->obs.data[i]=data0;
    for (i=0;i<rtcm->nav.n;i++) rtcm->nav.eph [i]=eph0;
    for (i=0;i<rtcm->nav.ng;i++) rtcm->nav.geph[i]=geph0;
    return 1;
}
/* free rtcm control ----------------------------------------------------------
