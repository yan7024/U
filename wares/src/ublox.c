...
#define ID_NAVPVT   0x0107      /* ubx message id: nav pvt (ZED-F9x; seeds raw->time for RAWX week=0) */
...
static int decode_rxmrawx(raw_t *raw)
{
    ...
    if (week==0) {
        /* Cold start / time not yet valid in RAWX: reuse last GPST from NAV or prior epoch */
        int wkfb = 0;
        if (raw->time.time != 0) {
            (void)time2gpst(raw->time, &wkfb);
        }
        if (wkfb < 1024) {
...
/* decode UBX-NAV-PVT: position / velocity / time (ZED-F9x) ------------------
 * Seeds raw->time from UTC calendar when valid — required before RXM-RAWX week=0
 * can be folded into GPST (decode_rxmrawx returns 0 if raw->time unset).
 *-----------------------------------------------------------------------------*/
static int decode_navpvt(raw_t *raw)
{
    ...
    /* Payload must reach nano (offset 16..19 from payload start) */
    if (raw->len < 8 + 22) {
        trace(2,"ubx nav-pvt length error: len=%d\n", raw->len);
        return -1;
    }
    valid = (int)U1(p + 11);
    if ((valid & 3) != 3) {
        /* validDate + validTime not both set */
        return 0;
    }
    ep[0] = (double)U2(p + 4);
    ep[1] = (double)U1(p + 6);
    ...
    ep[3] = (double)U1(p + 8);
    ep[4] = (double)U1(p + 9);
    ep[5] = (double)U1(p + 10) + (double)I4(p + 16) * 1e-9;

    raw->time = utc2gpst(epoch2time(ep));
    if (raw->outtype) {
        sprintf(raw->msgtype, "UBX NAV-PVT  (%4d):", raw->len);
    }
    return 0;
}
