...
static int decode_ssr1(rtcm_t *rtcm, int sys, int subtype)
{
    ...
    for (j=0;j<nsat&&i+121+np+ni+nj<=rtcm->len*8;j++) {
        ...
        rtcm->nav.ssr[sat-1].t0 [0]=rtcm->time;
        rtcm->nav.ssr[sat-1].udi[0]=udint;
        rtcm->nav.ssr[sat-1].iod[0]=iod;
        rtcm->nav.ssr[sat-1].iode=iode;     /* SBAS/BDS: toe/t0 modulo */
        rtcm->nav.ssr[sat-1].iodcrc=iodcrc; /* SBAS/BDS: IOD CRC */
        rtcm->nav.ssr[sat-1].refd=refd;
        
        for (k=0;k<3;k++) {
            rtcm->nav.ssr[sat-1].deph [k]=deph [k];
            rtcm->nav.ssr[sat-1].ddeph[k]=ddeph[k];
        }
        rtcm->nav.ssr[sat-1].update=1;
    }
    return sync?0:10;
}
...
static int decode_ssr2(rtcm_t *rtcm, int sys, int subtype)
{
    ...
    for (j=0;j<nsat&&i+70+np<=rtcm->len*8;j++) {
        ...
        rtcm->nav.ssr[sat-1].t0 [1]=rtcm->time;
        rtcm->nav.ssr[sat-1].udi[1]=udint;
        rtcm->nav.ssr[sat-1].iod[1]=iod;
        
        for (k=0;k<3;k++) {
            rtcm->nav.ssr[sat-1].dclk[k]=dclk[k];
        }
        rtcm->nav.ssr[sat-1].update=1;
    }
    return sync?0:10;
...
static int decode_ssr3(rtcm_t *rtcm, int sys, int subtype)
{
    ...
    for (j=0;j<nsat&&i+5+np<=rtcm->len*8;j++) {
        ...
        rtcm->nav.ssr[sat-1].t0 [4]=rtcm->time;
        rtcm->nav.ssr[sat-1].udi[4]=udint;
        rtcm->nav.ssr[sat-1].iod[4]=iod;
        
        for (k=0;k<MAXCODE;k++) {
            rtcm->nav.ssr[sat-1].cbias[k]=(float)cbias[k];
        }
        rtcm->nav.ssr[sat-1].update=1;
    }
    return sync?0:10;
...
static int decode_ssr4(rtcm_t *rtcm, int sys, int subtype)
{
    ...
    for (j=0;j<nsat&&i+191+np+ni+nj<=rtcm->len*8;j++) {
        ...
        rtcm->nav.ssr[sat-1].t0 [0]=rtcm->nav.ssr[sat-1].t0 [1]=rtcm->time;
        rtcm->nav.ssr[sat-1].udi[0]=rtcm->nav.ssr[sat-1].udi[1]=udint;
        rtcm->nav.ssr[sat-1].iod[0]=rtcm->nav.ssr[sat-1].iod[1]=iod;
        rtcm->nav.ssr[sat-1].iode=iode;
        rtcm->nav.ssr[sat-1].iodcrc=iodcrc;
        rtcm->nav.ssr[sat-1].refd=refd;
...
static int decode_ssr5(rtcm_t *rtcm, int sys, int subtype)
{
    ...
    for (j=0;j<nsat&&i+6+np<=rtcm->len*8;j++) {
        ...
        rtcm->nav.ssr[sat-1].t0 [3]=rtcm->time;
        rtcm->nav.ssr[sat-1].udi[3]=udint;
        rtcm->nav.ssr[sat-1].iod[3]=iod;
        rtcm->nav.ssr[sat-1].ura=ura;
        rtcm->nav.ssr[sat-1].update=1;
    }
    return sync?0:10;
}
...
static int decode_ssr6(rtcm_t *rtcm, int sys, int subtype)
{
    ...
    for (j=0;j<nsat&&i+22+np<=rtcm->len*8;j++) {
        ...
        rtcm->nav.ssr[sat-1].t0 [2]=rtcm->time;
        rtcm->nav.ssr[sat-1].udi[2]=udint;
        rtcm->nav.ssr[sat-1].iod[2]=iod;
        rtcm->nav.ssr[sat-1].hrclk=hrclk;
        rtcm->nav.ssr[sat-1].update=1;
    }
    return sync?0:10;
}
...
static int decode_ssr7(rtcm_t *rtcm, int sys, int subtype)
{
    ...
    for (j=0;j<nsat&&i+5+17+np<=rtcm->len*8;j++) {
        ...
        rtcm->nav.ssr[sat-1].t0 [5]=rtcm->time;
        rtcm->nav.ssr[sat-1].udi[5]=udint;
        rtcm->nav.ssr[sat-1].iod[5]=iod;
        rtcm->nav.ssr[sat-1].yaw_ang =yaw_ang / 256.0*180.0; /* (deg) */
        rtcm->nav.ssr[sat-1].yaw_rate=yaw_rate/8192.0*180.0; /* (deg/s) */
        
        for (k=0;k<MAXCODE;k++) {
            rtcm->nav.ssr[sat-1].pbias[k]=pbias[k];
            rtcm->nav.ssr[sat-1].stdpb[k]=(float)stdpb[k];
        }
    }
    return 20;
}
